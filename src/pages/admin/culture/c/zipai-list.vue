    ...
    <Drawer :mask-closable="false" :title="formData.id ? '修改':'添加'" width="50%" v-model="isedit">
      <Form :model="formData" :label-width="80">
        <FormItem label="字派所在地">
          <Input
            v-model="formData.ziapiLocation"
            placeholder="输入字派所在地"
        ...
        <FormItem label="祖先名">
          <Input
            v-model="formData.ancestorsName"
            placeholder="输入祖先名"
            @keyup.enter.native="toSubmit"
          />
        </FormItem>
  ...
  methods: {
    ...
    toEdit(e) {
      if (!e) {
        ...
        this.api
          .get(this.api.admin.base + this.api.admin.culture_zipai_info, {
            id: e
          })
          .then(res => {
            ...
            let zipaiTxt = res.data.zipaiTxt.length
              ? res.data.zipaiTxt.split(";")
              : [];
    ...
    remove(index) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.api
            .get(this.api.admin.base + this.api.admin.culture_zipai_del, {
    ...
    toSubmit() {
      ...
      this.api
        .post(this.api.admin.base + this.api.admin.culture_zipai_edit, data)
        .then(res => {
          if (res.code === 200) {
            if (data.id) {
