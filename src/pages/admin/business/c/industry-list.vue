  ...
  <div>
    ...
    <Drawer :mask-closable="false" :title="formData.id ? '修改':'添加'" width="50%" v-model="isedit">
      <Form :model="formData" :label-width="80">
        ...
        <FormItem label="预览图">
          ...
          >
            <Button type="dashed">
              ...
              <div
                class="img"
                :style="formData.fanNewsUploadFileList.length ? api.imgBG(formData.fanNewsUploadFileList[0].filePath) :''"
                v-else-if="formData.fanNewsUploadFileList"
              ></div>
        ...
        <FormItem label="位于">
          <Input
            v-model="formData.industryLocation"
            placeholder="位于"
  ...
  methods: {
    ...
    toEdit(e) {
      ...
      if (!e) {
        ...
        this.api
          .get(this.api.admin.base + this.api.admin.industry_info, {
            id: e
          })
          .then(res => {
            if (res.code == 200) {
    ...
    remove(index) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.api
            .get(this.api.admin.base + this.api.admin.industry_del, {
    ...
    toSubmit() {
      ...
      this.api
        .post(this.api.admin.base + this.api.admin.industry_edit, data)
        .then(res => {
          if (res.code === 200) {
            if (data.id) {
    ...
    toDrft() {
      ...
      this.api
        .post(this.api.admin.base + this.api.admin.industry_drft, data)
        .then(res => {
          if (res.code === 200) {
            if (data.id) {
              this.$Message.success("修改成功");
